...
def make_meshgrid(N1, N2, xrange, yrange, tkwargs):
    Xplt, Yplt = torch.meshgrid(
        torch.linspace(xrange[0], xrange[1], N1, **tkwargs),
        torch.linspace(yrange[0], yrange[1], N2, **tkwargs),
    )
    xplt = torch.stack(
        (
...
def plot_3_graphs(X, feasible_inds_list, Xplt, Yplt, Z_list, title_list, constraints):
    ...
    for i in range(len(Z_list)):
        try:
            ...
            Z_list[i] = Z_list[i].detach().numpy()
    ...
    for i in range(3):
        ...
        ax[i].plot(
            X[feasible_inds_list[i], 0], X[feasible_inds_list[i], 1], "og", label="Good"
        )
        ax[i].plot(
            X[~feasible_inds_list[i], 0],
            X[~feasible_inds_list[i], 1],
            "or",
            label="Bad",
        )
...
def side_by_side(
    X, Xplt, Yplt, Z_list, N1, N2, title=None, feasible_inds=None, boundary=None
):
    ...
    for i in range(2):
        ...
        if feasible_inds == None:
            ...
            ax[i].plot(X[~feasible_inds, 0], X[~feasible_inds, 1], "or", label="Bad")
        if boundary != None:
