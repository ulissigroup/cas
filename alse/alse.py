...
class alse():
    ...
    def initialize_model(self, model_type, **kwargs):
        ...
        for i in range(len(model_type)):
            ...
            self.list_of_models.append(
                fit_gp_model(model_type[i], self.normalized_x, self.train_y[i], **kwargs)
            )
    ...
    def next_n_test_points(self, X, num_pred, Ytemp):
        for _ in range(num_pred):
            X, list_of_models = one_iter_eci(X, self.y_constraints, self.punchout_radius, self.x_bounds, Ytemp[0].detach(), Ytemp[1].detach(), Ytemp[2].detach())
