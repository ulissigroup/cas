...
def gen_test_point_for_accuracy(pos_overlap, candidates, num_points, N, threshold=0.1):
    ...
    all_in_bound_points = torch.masked_select(
        candidates, pos_overlap & dist_mask
    )  # Returns a 1D tensor
    all_out_of_bound_points = torch.masked_select(candidates, ~pos_overlap & dist_mask)
    # Reshape to correct shape
    in_bound_cand = all_in_bound_points.reshape(
        (pos_overlap & dist_mask).sum(), candidates.shape[1]
    )
    out_of_bound_cand = all_out_of_bound_points.reshape(
        (~pos_overlap & dist_mask).sum(), candidates.shape[1]
    )
    # Select from available candidates
    in_bound_tpoints = in_bound_cand[
        torch.randint(in_bound_cand.shape[0], (num_points // 2,))
    ]
    # In case odd number points requested
    out_of_bound_tpoints = out_of_bound_cand[
        torch.randint(out_of_bound_cand.shape[0], (num_points // 2 + num_points % 2,))
    ]
...
def _boundary_surface(candidates, mask):
    return torch.masked_select(candidates, mask).reshape(
        mask.sum(), candidates.shape[1]
    )
