...
def read_excel():
    ...
    path1 = "../test_data/4340_2.5mm_5.24gmin.xlsx"
    ...
    # Hardcoded
    ones = torch.ones(40)
    print("Spot size is hardcoded in this version")
    ...
    _height = (
        parse_data(consolidated_data, "heights avg (mm)").bool().long().unsqueeze(-1)
    )
    adhere = _height

    X = torch.stack((power, velocity, spot_size), -1)
